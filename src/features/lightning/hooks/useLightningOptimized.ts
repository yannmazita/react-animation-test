// src/features/lightning/hooks/useLightningOptimized.ts
 ...
 * }
 */
export const useLightningOptimized = (
  ...
  useEffect(() => {
    ...
    /**
     * Renders a single frame of the lightning animation using multi-pass rendering
     * for efficient glow effects.
     *
     ...
     * 3. Renders glow effect using multiple passes:
     *    - Pass 1: Wide glow stroke with low opacity (0.3)
     *    - Pass 2: Medium glow stroke with medium opacity (0.5)
     *    - Pass 3: Core lightning with full opacity
     * 4. Draws all trails and active bolts in a single batched stroke operation,
     *    with trails rendered at decreasing opacity based on age
     *
     * This approach significantly reduces GPU usage compared to native shadowBlur
     * while maintaining visual quality through layered rendering.
     *
    ...
    const drawFrame = (deltaTime: number) => {
      ...
      // Disable shadow for trails, only use for active bolts
      ctx.shadowBlur = 0;

      // Batch similar trails by opacity ranges
      const opacityBuckets = new Map<number, typeof trails>();

      for (const trail of trails) {
        const opacity = 1 - trail.age / trailDuration;
        const bucketKey = Math.floor(opacity * 10) / 10; // Round to nearest 0.1
      ...
      for (const [opacity, trailBatch] of opacityBuckets) {
        ...
        for (const trail of trailBatch) {
          ...
          // Add a break in the path to prevent connecting trails
          ctx.moveTo(
      ...
      // Draw active bolts with minimal blur
